...
// Kongduino
void SX127x::setupLoRandom() {
  // First save current settings
  modemconf0 = this->mod->SPIgetRegValue(RegOpMode);
  delay(10);
  modemconf1 = this->mod->SPIgetRegValue(RegModemConfig1);
  delay(10);
  modemconf2 = this->mod->SPIgetRegValue(RegModemConfig2);
  delay(10);
  ...
  this->mod->SPIsetRegValue(RegOpMode, 0b10001101);
  delay(10);

  // 0111: BW 125
  // 001: CR 4/5
  // 0: Explicit Header mode
  this->mod->SPIsetRegValue(RegModemConfig1, 0b01110010);
  delay(10);
  ...
  // 00: RX Time-Out MSB
  this->mod->SPIsetRegValue(RegModemConfig2, 0b01101000);
  delay(10);
}

void SX127x::resetLoRa() {
  // reset settings
  this->mod->SPIsetRegValue(RegOpMode, modemconf0);
  delay(10);
  this->mod->SPIsetRegValue(RegModemConfig1, modemconf1);
  delay(10);
  this->mod->SPIsetRegValue(RegModemConfig2, modemconf2);
  delay(10);
}

uint8_t SX127x::getLoRandomByte() {
  uint8_t x = 0, b;
  for (uint8_t j = 0; j < 8; j++) {
    b = this->mod->SPIgetRegValue(RegRssiWideband) & 0b00000001;
    while (b == this->mod->SPIgetRegValue(RegRssiWideband) & 0b00000001) {
...
void SX127x::fillRandom() {
  fillRandom(randomStock, 256);
  randomIndex = 0;
}

// Kongduino
void SX127x::fillRandom(uint8_t *buffer, uint16_t ln) {
  setupLoRandom();
